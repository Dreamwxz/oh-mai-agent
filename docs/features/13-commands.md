...
命令的返回文本经 **`ReplySender.send_raw` 直发出口**（commands.py 的 `cmd_reply`）直发原文，**不经过 LLM 润色**——命令是排障/操作场景，任务 ID、状态等关键信息不能被改写。命令响应与任务回复共享同一发送实现，因此同样获得指数退避重试、静默掉包检测与分割保障；区别是命令响应**不写入 MaiBot 上下文**（发送出口纯发送原则，命令回显属操作输出而非认知内容）。这意味着命令响应是即时、原始、确定性的，与任务回复的"加工后"风格形成对照——区别在"是否润色、是否入上下文"，发送可靠性是一致的。
