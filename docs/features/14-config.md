...
分割在润色之后进行（`ReplySender` 的 `_split`），两级策略确定性切分，保留原文不丢内容：**含换行时按行分割为主**（行是打包的基本单元，段边界只在行边界，仅当单行超过 `max_length` 时才在行内按句号切分），**无换行（单段长文）时整段按句号分割**；所有发送路径（instant 任务、agent 完成回复、send_message 工具、失败通知、命令回复）统一跟随本配置生效，不再提供按消息覆盖参数。详见 [回复润色](./06-polish.md)。
