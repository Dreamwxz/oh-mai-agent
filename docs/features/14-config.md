...
分割在润色之后进行（`ReplySender` 的 `_split`），按行/句末标点确定性切分，保留原文不丢内容；所有发送路径（instant 任务、agent 完成回复、send_message 工具、失败通知、命令回复）统一跟随本配置生效，不再提供按消息覆盖参数。详见 [回复润色](./06-polish.md)。
