...
**与 ask_user 的边界**。ask_user 提问消息也走统一发送入口（`send_final_reply`，`TaskManager._ask_callback`）——提问默认润色（项目的优势是"更像人"，提问同样以聊天口吻呈现），并获得与回复相同的重试、掉包检测与上下文记录保障。与回复路径的区别只有两点：提问不创建 instant 任务（是 Agent 循环内的双向交互，等待用户回复后继续，而非任务结束后的单向推送）；命令响应（`cmd_reply`）以 `polish=False` 直发不润色（排障场景需确定性，见 [命令](./13-commands.md)）。详见 [AI 提问](./07-ask-user.md)。
