...
**核心模块**：`executor/instant.py`（润色、发送、失败处理全部在此）。
...
`InstantExecutor.execute()`（executor/instant.py:483-589）是整个回复路径的终点：先 `send_final_reply` 润色发送，再判终态收尾。整个执行路径不跨进程，上下文经 `executor/context.py` 的 `current_task` ContextVar 传递。执行结果（COMPLETED / FAILED）通过 `scheduler.on_task_completed()` 回调通知调度器释放额度，不经过命令总线的事件分发——instant 任务没有 AgentLoop 那样的事件监听循环，终态通知是直调的。

**PolishService：黑话匹配评分**。`PolishService`（executor/instant.py:81-305）是润色核心，三件事：
...
2. `_match_jargons()` 机械子串匹配黑话——复刻 MaiBot `jargon_context_matcher` 的匹配与评分：`_jargon_in_scope`（instant.py:41）按 `is_global` 或 `session_id_dict` 过滤黑话范围，`_calculate_match_score`（instant.py:57）复刻高频词加权评分（命中高频词加 1000 分基数、按出现次数加权、按消息位置微调），取前 `MAX_JARGON_REFERENCE_MATCHES`（10）条；
3. 把上下文、黑话与原始结果交给 `prompt_service.build("polish")` 渲染系统提示词，`ctx.llm.generate(model="replyer", timeout_ms=120000)` 润色一次（instant.py:153-160）。任何异常记录 warning 后返回原文。
...
**回复模式：reply vs relay**。`send_final_reply` 接受 `kind` 参数（instant.py:322），默认 `"reply"`，可选 `"relay"`。reply 模式将回复视为"我自己的回复"；relay 模式将回复视为"转达他人的话"，提示词会调整为"替某人转达"。`requester` 参数提供转达委托人的展示名，由 `InstantExecutor._resolve_requester()`（instant.py:533-556）从 `task.owner` 的 `platform:user_id` 格式解析——通过 `ctx.chat.get_all_streams` 匹配用户后取 `user_nickname` 或 `user_cardname`。解析失败时回退空串，`requester` 为空时 relay 模式退化为跟 reply 模式一样的润色效果。这个区分主要是为了 send_message 工具：当 Agent 替用户发消息时，回复应该以"这是某人的转达"的口吻呈现，而不是"这是我说的"。

**send_final_reply：润色一次 + 指数退避重试**。`send_final_reply()`（instant.py:311-420）是发送入口，两段式：先 `svc.polish()` 润色一次（绝不重试，异常由 PolishService 内部消化），再循环发送。发送阶段是 `for attempt in range(max_retries)` 的循环：每次尝试调用 `ctx.send.text(polished, stream_id)`，成功后立即向目标流写入两条上下文记录（纯文本 + XML 动机注释，见下文），然后 return。失败时按 `2^attempt` 指数退避 sleep，间隔 1s → 2s，最多 `max_retries=3` 次，循环耗尽后抛出最后一个异常交由上层标记 FAILED。退避间隔短，是因为回复是用户等待中的即时消息，不值得为一次发送等待过久；重试次数有限，是因为发送失败大概率是目标流本身的问题，重试再多也是徒劳。
...
**静默掉包检测**。MaiBot SDK 的 `ctx.send.text()` 在底层发送失败但不抛异常时可能返回 `False` 或 `None`（如 API 返回空响应但状态码 200）。`send_final_reply` 每次尝试后显式检查返回值（instant.py:370），检测到即转成 `RuntimeError` 触发重试。如实说明：这是对 SDK 边界行为的**防御性检测**，不是系统设计特性——若全部 3 次重试都遇到掉包，消息确实发不出去，任务最终标记 FAILED。检测逻辑封装在 `send_final_reply` 内部，外部调用方（InstantExecutor、send_message 工具、Planner 的 send_message API）无需自行处理。

**发送成功后的上下文记录**。发送成功后立即向目标流写上下文记录，这一步由 `send_final_reply` 自身在 `ctx.send.text` 返回成功值后执行。写两条记录，写入失败都不影响整体流程，只打 warning 日志：

- **纯文本记录**：实际发送的润色后文本，无 XML 标签，通过 `ctx.maisaka.context.append` 写入，`source_kind` 标记为 `plugin:oh-mai-agent:task-reply`。供后续 LLM 推理看到这条回复确实发出去了。
- **XML 动机注释**：由 `_append_motivation_note()`（instant.py:558-589）对跨流回复（`task.reply_stream_id` 或 `metadata["_is_reply"]`）单独补写。用 `context_note` 模板渲染任务动机生成 XML 标签，写入目标流，让从别处发来的任务结果在聊天上下文里可追溯。迁移后该分支由父进程直接写：`execute()` 调用 `send_final_reply` 不再传 motivation，统一收口到 `_append_motivation_note`。判断条件是：只要 `reply_stream_id` 不为 None 或 `_is_reply` 为 True，就补写——这比单纯看 `reply_stream_id` 更宽，覆盖了通过 `_dispatch_reply_instant` 创建的回复任务。

**成功与失败的终态收尾**。发送成功后，`execute()` 重新 `store.get` 判终态（防并发终态覆盖），非终态才走 `complete_and_notify`（executor/base.py:114-118）：`transition(COMPLETED)`（竞争时 `force` 兜底）→ 落库 → `scheduler.on_task_completed()` 释放并发额度。异常路径走 `fail_task()`（instant.py:426-477）：先做双重终态守卫——本地 `is_terminal()` 直接返回，再重载持久化记录检查（防进程内快照过期）——随后可选 `send_message` 先发"任务执行失败"通知（失败通知本身发送失败也不影响状态更新，异常静默吞掉），再 `transition(FAILED)`、被状态机拒绝时 `force(FAILED)` 兜底，落盘并通知调度器。终态守卫的意义在于：任务可能已被调度器超时判 FAILED 或用户取消，执行器不能用一个过期的进程内快照覆盖并发终态。`fail_task` 的 `send_message` 可选特性也说明：失败通知不是必须的——`execute()` 异常路径总是 `send_message=True`，但其他调用方（如 AgentLoop 的异常处理）可以选择不发消息，只悄悄地标记 FAILED。

**调用路径汇总**。`send_final_reply` 是润色 + 发送的唯一入口，被四类调用方复用：

1. **instant 任务**：`InstantExecutor.execute()`（instant.py:490）直接调用，这是最主流的路径。
...
**面向使用者的回复路径**。普通用户不需要关心润色细节，只需知道：任务完成后，最终回复会自动经过润色与重试后直发到目标聊天流。通过 `send_message` 工具（Agent 循环内或 Planner 侧）主动发消息时，同样自动走润色。若想跳过润色或关闭黑话，改 `use_jargon` 即可。回复是单向推送，用户无需操作；唯一特殊的是 ask_user 提问（见 [AI 提问](./07-ask-user.md)），那是需要用户回应的双向交互。
