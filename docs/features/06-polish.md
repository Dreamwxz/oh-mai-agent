...
**与 ask_user 的边界**。ask_user 提问消息也走统一发送入口（`send_final_reply`，`TaskManager._ask_callback`）——提问默认润色（项目的优势是"更像人"，提问同样以聊天口吻呈现），并获得与回复相同的重试、掉包检测与上下文记录保障。唯一区别是提问不创建 instant 任务：它是 Agent 循环内的双向交互（等待用户回复后继续），而非任务结束后的单向推送。详见 [AI 提问](./07-ask-user.md)。

**命令响应的例外**。命令响应（`cmd_reply`，见 [命令](./13-commands.md)）同样走统一发送入口，但以 `polish=False` 直发**不润色**——命令是排障/操作场景，任务 ID、状态等关键信息不能被 LLM 改写。可靠性保障（重试、掉包检测、上下文记录）与润色消息完全一致，区别只在是否经过 LLM。
