...
插件重启后，`TaskRecovery.recover()`（`domain/recovery.py`）按持久化状态决定恢复动作：SCHEDULED 重新入队等待定时器触发；RUNNING 经 `force()` 降级为 PENDING 重新排队并经 `mark_recovered_from_running()` 打标记（键 `META_RECOVERED_FROM_RUNNING`）；WAITING_INPUT 保持状态，旧 asyncio.Event 已随进程消失，等待用户通过 Hook 重新唤醒；PAUSED 与终态不做任何操作。
