...
关键点：第 3 步的 `_scheduler_executor` 闭包（`lifecycle.py:72-79`）经 `plugin._executor_ref["tm"]` 引用 TaskManager，打破「调度器 → 执行器 → 任务管理器 → 调度器」的循环依赖；第 5 步 `setup()`（`core/task_manager.py:148-226`）注册任务管理工具、info、file（role_provider 取 current_task 角色）、ask_user、send_message 与跨插件 API 工具。
